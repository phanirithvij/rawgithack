...
(function (doc) {
  ...
  function formatURL() {
    ...
    if (ghUrl) {
      var matches = ghUrl.match(/^(\w+:\/\/(raw).armyofrats.in\/([^\/]+)\/([^\/]+))\/([^\/]+)\/(.*)/i);
      if (!matches) {
        devEl.value = ghUrl;
